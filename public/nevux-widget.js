(function () {
  ...
  const API_BASE = "https://nevux.app"; // Cambiá esto por tu dominio real de Vercel
  ...
  // ─── Extraer IDs de Tiendanube ───
  function detectStoreId() {
    // Tiendanube expone el store_id en varios lugares
  ...
  fetch(`${API_BASE}/api/widget-render?store_id=${storeId}${productId ? `&product_id=${productId}` : ""}`)
    ...
    .then((data) => {
      ...
      data.widgets.forEach((widget) => {
        ...
        // Acá después agregamos los otros 14 widgets
      });
    })
  ...
  function renderCountdown(widget) {
    ...
    if (!target) {
      // Fallback: insertar antes del botón de compra
      target = qs('button[data-store="product-buy-button"]') || qs('.js-add-to-cart');
    }
    if (!target) {
      console.warn("[Nevux] No se encontró ubicación para el contador");
      return;
    }
  ...
  function updateCountdown(container, cfg) {
    ...
    if (diff <= 0) {
      container.innerHTML = `
        <div style="text-align:center;padding:${cfg.widget_padding || 15}px;background:${cfg.background_color || '#1e1e1e'};border-radius:${cfg.widget_border_radius || 5}px;color:#ef4444;font-weight:800;font-size:${cfg.title_font_size || 16}px;">
          ⏰ ¡La oferta terminó!
        </div>`;
    ...
    const units = [
      { val: days, label: "DÍAS", show: showDays },
    ...
    const titleHtml = cfg.title ? `
      <div style="margin-bottom:${cfg.subtitle ? '4px' : '12px'};text-align:${align};">
        <span style="font-size:${tSize}px;font-weight:800;color:${titleColor};">${cfg.title}</span>
      </div>` : '';

    const subtitleHtml = cfg.subtitle ? `
      <div style="display:inline-block;margin-bottom:12px;padding:3px 10px;background:${subBg};border-radius:5px;text-align:${align};">
        <span style="font-size:${sSize}px;font-weight:600;color:${subColor};">${cfg.subtitle}</span>
      </div>` : '';
    ...
    const clockHtml = units.map((u, i) => {
      ...
      return `
        <div style="display:flex;flex-direction:column;align-items:center;gap:4px;">
          <div style="background:${clockBg};border-radius:${cRadius}px;padding:${Math.max(4, cSize * 0.3)}px ${Math.max(6, cSize * 0.5)}px;min-width:${cSize * 2.2}px;text-align:center;box-shadow:0 2px 6px rgba(0,0,0,0.25);">
            <span style="font-size:${cSize}px;font-weight:800;color:${numColor};font-variant-numeric:tabular-nums;letter-spacing:0.05em;">${val}</span>
          </div>
          ${showLabels ? `<span style="font-size:${Math.max(9, cSize * 0.45)}px;font-weight:700;color:${titleColor}aa;text-transform:uppercase;letter-spacing:0.12em;">${u.label}</span>` : ''}
        </div>
        ${i < units.length - 1 ? `<span style="font-size:${cSize * 0.9}px;font-weight:800;color:${titleColor}66;margin-top:${showLabels ? '-14px' : '0'};">:</span>` : ''}
      `;
    }).join('');

    container.innerHTML = `
      <div style="padding:${pad}px;background:${bg};border-radius:${radius}px;text-align:${align};">
        ${titleHtml}
        ${subtitleHtml}
        <div style="display:flex;justify-content:${alignCss};align-items:flex-start;gap:6px;">
          ${clockHtml}
        </div>
      </div>
    `;
