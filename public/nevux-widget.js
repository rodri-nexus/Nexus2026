...
/* ═══════════════════════════════════════════
  ...
  function renderExtrasInterruptor(w) {
    ...
    var mostrarBadge = cfg.mostrarBadge !== false && badge !== "";
    var colorBadge = cfg.colorBadge || "#dc2626";
    var colorTextoBadge = cfg.colorTextoBadge || "#ffffff";
    ...
    var colorPrecio = cfg.colorPrecio || "#111827";
    var colorBorde = cfg.colorBorde || "#fcd34d";
...
/* ═══════════════════════════════════════════
  ...
  function renderComparadorMarca(w) {
    ...
    div.className = "nvx-root-comparador";
    ...
    if (pageType === "product") {
      ...
      if (opinTarget && opinTarget.parentNode) {
        opinTarget.parentNode.insertBefore(div, opinTarget);
      } else {
        var productSectionSelectors = [
          "[data-store='home-featured-products']",
          ".section-featured-home",
          ".section-products-home",
          ".js-home-products-container",
          ".js-products-section",
        ...
        for (var i = 0; i < productSectionSelectors.length; i++) {
          var el = document.querySelector(productSectionSelectors[i]);
          if (el) { prodSection = el; break; }
        }
        if (prodSection && prodSection.parentNode) {
          if (prodSection.nextSibling) {
            prodSection.parentNode.insertBefore(div, prodSection.nextSibling);
          } else {
            prodSection.parentNode.appendChild(div);
          }
        } else {
          var fallbackTarget = document.querySelector("[data-store='home-newsletter']") ||
                               document.querySelector(".section-newsletter") ||
                               document.querySelector("footer") ||
                               document.querySelector("#footer");
          if (fallbackTarget && fallbackTarget.parentNode) {
            fallbackTarget.parentNode.insertBefore(div, fallbackTarget);
          } else {
...
/* ═══════════════════════════════════════════
  ...
  function renderMediosPago(w) {
    ...
    var campaignTheme = cfg.campaignTheme || "none";
    var THEMES = {
      "black-friday": { bgColor: "#111827", borderColor: "#F59E0B", textColor: "#FFFFFF", accentColor: "#F59E0B" },
      "hot-sale": { bgColor: "#0F172A", borderColor: "#EF4444", textColor: "#FFFFFF", accentColor: "#EF4444" },
      "cyber-monday": { bgColor: "#090D16", borderColor: "#3B82F6", textColor: "#FFFFFF", accentColor: "#3B82F6" },
      "navidad": { bgColor: "#064E3B", borderColor: "#10B981", textColor: "#FFFFFF", accentColor: "#EF4444" },
      "san-valentin": { bgColor: "#831843", borderColor: "#FB7185", textColor: "#FFFFFF", accentColor: "#F43F5E" },
      "dia-madre-padre": { bgColor: "#312E81", borderColor: "#6366F1", textColor: "#FFFFFF", accentColor: "#10B981" },
      "sale-liquidacion": { bgColor: "#7F1D1D", borderColor: "#FBBF24", textColor: "#FFFFFF", accentColor: "#FBBF24" }
    };
...
/* ═══════════════════════════════════════════
  ...
  function renderTablaTalles(w) {
    ...
    var textoBotonElegir = cfg.textoBotonElegir || "Elegir este talle";
    var tituloModal = cfg.tituloModal || "GUÍA DE TALLES Y MEDIDAS";
    var subtextoModal = cfg.subtextoModal || "Seleccioná tu talle ideal para agregarlo a tu compra";
    ...
    var theadCols = '<th style="width: 40px;">Elegir</th>' + columnas.map(function(c) {
      return '<th>' + escapeHtml(c) + '</th>';
    }).join("");

    var currentSelectedTalle = savedSize || (filas[0] ? filas[0].talle : "");

    var tbodyRows = filas.map(function(f, idx) {
      var talleVal = (f.talle || "").trim();
      var isChecked = talleVal === currentSelectedTalle;
      var activeClass = isChecked ? "nvx-talle-row nvx-row-active" : "nvx-talle-row";

      return `
        <tr class="${activeClass}" data-talle="${escapeHtml(talleVal)}">
          <td>
            <input type="radio" name="nvx_talle_radio_${w.id}" class="nvx-talle-radio" value="${escapeHtml(talleVal)}" ${isChecked ? 'checked' : ''} />
          </td>
      ...
      <div id="nvx-talles-modal-card-${w.id}">
        <div class="nvx-modal-header">
          <div>
            ...
            <p class="nvx-modal-subtext">${escapeHtml(subtextoModal)}</p>
          </div>
        ...
        <button type="button" class="nvx-save-talle-btn" id="nvx-save-talle-btn-${w.id}">
          ✓ ${escapeHtml(textoBotonElegir)} (<span id="nvx-current-talle-text-${w.id}">${escapeHtml(currentSelectedTalle)}</span>)
        </button>
    ...
    var saveBtn = modal.querySelector("#nvx-save-talle-btn-" + w.id);
    var currentTalleLabel = modal.querySelector("#nvx-current-talle-text-" + w.id);
    ...
    rows.forEach(function(row) {
      row.addEventListener("click", function() {
        var talle = row.getAttribute("data-talle");
        currentSelectedTalle = talle;

        rows.forEach(function(r) { r.classList.remove("nvx-row-active"); });
        row.classList.add("nvx-row-active");

        var radio = row.querySelector(".nvx-talle-radio");
        if (radio) radio.checked = true;

        if (currentTalleLabel) currentTalleLabel.textContent = talle;
      });
    });
    ...
    saveBtn.addEventListener("click", function(e) {
      ...
      try {
        localStorage.setItem(storageKey, currentSelectedTalle);
      } catch(err) {}
      ...
      saveBtn.textContent = "¡Talle " + currentSelectedTalle + " Guardado! ✓";
      saveBtn.style.background = "#047857";

      setTimeout(function() {
        saveBtn.innerHTML = `✓ ${escapeHtml(textoBotonElegir)} (<span id="nvx-current-talle-text-${w.id}">${escapeHtml(currentSelectedTalle)}</span>)`;
...
/* ═══════════════════════════════════════════
  ...
  /* ═══════════════════════════════════════════
     RENDER PACK COMPLEMENTARIOS (v2 - Motor Nativo)
  ═══════════════════════════════════════════ */
  function renderPackComplementarios(w) {
    ...
    // Esperar a que el contenedor visible del producto esté listo
    var attempts = 0;
    var timer = setInterval(function() {
      ...
      if (buyBtn) {
        var f = buyBtn.closest("form");
        if (f && !f.classList.contains("js-ajax-cart-panel")) {
          target = f;
        } else {
          target = buyBtn.parentElement;
        }
      }

      if (!target) {
        var forms = document.querySelectorAll("form.js-product-form, form[action*='/comprar'], form[action*='/cart']");
        for (var i = 0; i < forms.length; i++) {
          if (!forms[i].classList.contains("js-ajax-cart-panel") && !forms[i].closest(".js-ajax-cart-container")) {
            target = forms[i];
            break;
          }
        }
      ...
      if (!target && attempts >= 20) {
        target = document.querySelector("main") || document.querySelector("#main") || document.body;
      }
        ...
        if (buyBtn) {
          buyBtn.addEventListener("click", function(e) {
            ...
            selectedSet.forEach(function(idx) {
              var it = items[idx];
              if (it && it.variantId) {
                var cleanId = String(it.variantId).replace(/\D/g, "");
                if (cleanId) variantsToAdd.push(cleanId);
              }
            });

            // Si falta alguno, usar el actual
            if (variantsToAdd.length === 0) {
              var curr = document.querySelector("input[name='add_to_cart']");
              if (curr && curr.value) variantsToAdd.push(curr.value);
            }

            if (variantsToAdd.length === 0) {
              alert("Por favor elegí los productos desde el panel de Nevux usando 'Elegir de mi tienda'.");
              return;
            }
            ...
            // Motor de envío secuencial nativo
            var index = 0;
            function sendNext() {
              if (index >= variantsToAdd.length) {
                // Al terminar todos, abrir el modal nativo o redirigir
                setTimeout(function() {
